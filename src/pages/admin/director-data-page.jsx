...
function DirectorDataPage() {
  ...
  const handleMarkMultipleDone = async () => {
    if (selectedHistoryItems.length === 0) {
      setSuccessMessage(t('messages.selectAtLeastOne'));
      return;
    }
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
          <div className="flex items-center justify-center mb-4">
            ...
            <h2 className="text-xl font-bold text-gray-800">{t('messages.confirmMarkDone')}</h2>
          </div>

          <p className="text-gray-600 text-center mb-6">
            {t('messages.confirmMarkDone')}
          </p>
  ...
  const fetchSheetData = async () => {
    try {
      ...
      const response = await fetch(`https://docs.google.com/spreadsheets/d/1gTDJigoeyXYzxR7HxDMwOjGL0hPP3Qc-P9HAChNCxUw/gviz/tq?tqx=out:json&sheet=${CONFIG.SHEET_NAME}`);
      ...
      <div className="space-y-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <h1 className="text-2xl font-bold tracking-tight text-purple-700">
            {showHistory ? t('nav.history') + " " + t('nav.director') : t('nav.director')}
          </h1>
          ...
          <div className="flex space-x-4">
            ...
            {!showHistory && (
              ...
              >
                {isSubmitting ? t('common.submitting') : `${t('common.submit')} (${selectedItems.length})`}
              </button>
            )}
              ...
              <div className="fixed top-40 right-10 z-50">
                ...
                >
                  {markingAsDone
                    ...
                    : `${t('common.confirm')} ${selectedHistoryItems.length} ${t('assignTask.completedTasks')}`
                  }
                </button>
              </div>
            )}
        ...
        <div className="rounded-lg border border-purple-200 shadow-md bg-white overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="bg-red-50 p-4 rounded-md text-red-800 text-center">
              {error} <button className="underline ml-2" onClick={() => window.location.reload()}>{t('common.retry')}</button>
            </div>
          ) : showHistory ? (
            // History Table
            <>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex items-center">
                        <label htmlFor="start-date" className="text-sm text-gray-700 mr-1">{t('common.from')}</label>
                        <input
                          id="start-date"
                          type="date"
                          value={startDate}
                      ...
                      <div className="flex items-center">
                        <label htmlFor="end-date" className="text-sm text-gray-700 mr-1">{t('common.to')}</label>
                        <input
                          id="end-date"
                          type="date"
                          value={endDate}
                    ...
                    <tr>
                      ...
                      {sheetHeaders.slice(1, 13).map((header) => (
                        <th
                          ...
                          `}
                        >
                          {t(`table.${header.label.toLowerCase().replace(/ /g, '')}`) !== `table.${header.label.toLowerCase().replace(/ /g, '')}` ? t(`table.${header.label.toLowerCase().replace(/ /g, '')}`) : header.label}
                        </th>
                      ))}
                  ...
                  {filteredAccountData.length > 0 ? (
                          ...
                          {account.image ? (
                            ...
                            <label className={`flex items-center cursor-pointer ${account['col10']?.toUpperCase() === "YES" ? "text-red-600 font-medium" : "text-purple-600"} hover:text-purple-800`}>
                              ...
                              <span className="text-xs">
                                {account['col10']?.toUpperCase() === "YES" ? t('delegation.requiredUpload') : t('delegation.uploadReceiptImage')}
                    ...
                    <tr>
                      <td colSpan={sheetHeaders.length + 3} className="px-6 py-4 text-center text-gray-500">
                        {searchTerm ? t('delegation.noTasksFound') : t('delegation.noPendingTasks')}
                      </td>
                    </tr>
                  )}
                </tbody>
